  ...
  methods: {
    filter: function() {
      for (var j in this.lenderData) {
        ...
        //check lender Policy
        if (this.LVR>this.lenderData[j].maxLVRExcLMI){this.lenderData[j].eligible=false}
        if ((this.userData.proposedLoan.securityType=="Established House"||this.userData.proposedLoan.securityType=="New House")&&this.lenderData[j].acceptableSecurity.establishedDwelling==false){this.lenderData[j].eligible=false}
        if (this.userData.proposedLoan.securityType=="Vacant Land"&&this.lenderData[j].acceptableSecurity.vacantLand==false){this.lenderData[j].eligible=false}
        if (this.userData.proposedLoan.securityType=="Construction"&&this.lenderData[j].acceptableSecurity.construction==false){this.lenderData[j].eligible=false}
        var daysInEmploy = this.calculateMonths(this.userData.employment.dateStarted);
        //Check products
        for (var i in this.lenderData[j].products) {
          this.lenderData[j].products[i].eligible=true;
          if (this.lenderData[j].eligible== false){this.lenderData[j].products[i].eligible=false};
        }
      }
    },
