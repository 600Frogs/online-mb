...
export default {
  ...
  methods: {
    calculateMaxLoan(lender, product) {
      var surplus = 0;
      surplus = this.calculateIncome(this.userData, product) - this.calculateExpenses(lender) - this.calculateLivingExpenses(lender, product)-this.calculateRent();
    ...
    calculateRent: function(){
      var rent = 0;
      if (this.userData.residential.currentArrangement == "Renting through a Real Estate Agent" || this.userData.residential.currentArrangement == "Renting - Privately" ||
        this.userData.residential.currentArrangement == "Boarding i.e. with parents or family" || this.userData.residential.currentArrangement == "Other") {
          rent = parseInt(this.userData.residential.weeklyRentPaid)*52/12;
          rent = Math.max(rent, 650);
          if (this.userData.proposedLoan.propertyType == "Owner Occupied" && this.userData.residential.rentingPostSettlement == "No"){
            rent = 0;
          }
      }
      return rent;
    },
    calculateAssessmentRepayments: function (principal, interest, assessmentRate, term, iOTerm){
    ...
    calculateExpenses(lender) {
      ...
      for (var i in this.userData.liabilities.additionalLiabilities) {
        ...
        if (liability.type=="Credit Card"){
          ...
          expenses+=this.calculateAssessmentRepayments(liability.moneyOwing, liability.interestRate, parseFloat(lender.assessmentRate), liability.termRemaining, liability.iOTermRemaining);
        } else {
    ...
    calculateIncome(app, product) {
      ...
      for (var i in app.income){
        ...
        if (app.income[i].incomeType == "Overtime" || app.income[i].incomeType == "Bonus/Commission" || app.income[i].incomeType == "Rental Income" ||
            app.income[i].incomeType == "Expected Rental Return on Purchase" || app.income[i].incomeType == "Other investment income" ||
            app.income[i].incomeType == "Second job PAYG" || app.income[i].incomeType == "Other"){
          inc = inc*0.8;
        }
      ...
      for (i in this.userData.liabilities.additionalLiabilities){
        var liability = this.userData.liabilities.additionalLiabilities[i];
        if (liability.type!="Home Loan" || liability.taxDeductible!= "Yes"){continue}
      ...
      //resolve and return
      income = -parseFloat(tax)+parseFloat(taxFree)+parseFloat(taxable)+parseFloat(offsetLiab)+parseFloat(offsetInv);
      return income/12 ;
    },
    calculateLivingExpenses(lender, product) {
      ...
      if (app1.numberOfApplicants=="Two" && app2.partnerOfApplicantOne=="Yes"){income += this.calculateIncome(app2, product)*12}
      if (app1.relationshipStatus == 'Single' || app1.relationshipStatus == 'Recently Divorced'){
        finder=0;
      } else {
        finder = 3;
      }
      ...
      if (app1.numberOfApplicants == 'Two' && app2.partnerOfApplicantOne=="No") {
        var income = this.calculateIncome(app2, product)*12;
        if (app2.relationshipStatus == 'Single' || app2.relationshipStatus == 'Recently Divorced'){
          finder=0;
        } else {
          finder = 3;
        }
        mle = lender.mle[finder];
        if (income<=42000){ //net figures
          hem += parseInt(mle.fifty);
        } else if (income<=55000){
          hem+=parseInt(mle.seventy);
        } else if (income<=75000){
      ...
      return Math.max(hem, this.userData.liabilities.livingExpenses.basicLivingExpenses);
    },
